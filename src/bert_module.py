...
class BertModule(pl.LightningModule):
    def __init__(self, args):
        ...
        self.encoder_linear = nn.Linear(self.args.hidden_size, 2)
        ...
        self.lm_loss = nn.NLLLoss(ignore_index=self.args.pad_id)
        self.class_loss = nn.CrossEntropyLoss()
        
        self.save_hyperparameters(args)
        
    def forward(self, src_ids):  # src_ids: (B, L)
    ...
    def step_decoder(self, trg_ids, src_ids, encoder_outputs):  # (B, T_L), (B, S_L), (B, S_L, d_h)
        ...
        for t in range(1, trg_len):
            ...
            if self.args.use_copy:
                vocab_dists = self.copy_mech(context_vecs, hidden[-1,:,:], input_embs, vocab_dists, attn_dists, src_ids)  # (B, V)
    ...
    def beam_search(self, src_ids, encoder_outputs):  # (S_L)  (S_L, d_h)
        ...
        for t in range(1, self.args.max_decoder_len):
            ...
            if self.args.use_copy:
                vocab_dists = self.copy_mech(context_vecs, hidden[-1,:,:], input_embs, vocab_dists, attn_dists, src_ids)  # (K, V)
        ...
        result = torch.LongTensor(result_queue.get().decoded)
    ...
    def training_step(self, batch, batch_idx):
        ...
        lm_loss = self.lm_loss(torch.log(lm_outputs[:, :-1]).contiguous().view(-1, self.args.vocab_size), trg_ids[:, 1:].contiguous().view(-1))
    ...
    def training_epoch_end(self, training_step_outputs):
        ...
        train_lm_preds = make_tokens(train_lm_preds, self.tokenizer, eos_token=self.args.sep_token, pad_token=self.args.pad_token)
        train_lm_trues = make_tokens(train_lm_trues, self.tokenizer, eos_token=self.args.sep_token, pad_token=self.args.pad_token)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        valid_lm_preds = make_tokens(valid_lm_preds, self.tokenizer, eos_token=self.args.sep_token, pad_token=self.args.pad_token)
        valid_lm_trues = make_tokens(valid_lm_trues, self.tokenizer, eos_token=self.args.sep_token, pad_token=self.args.pad_token)
    ...
    def test_epoch_end(self, test_step_outputs):
        ...
        test_lm_preds = make_tokens(test_lm_preds, self.tokenizer, eos_token=self.args.sep_token, pad_token=self.args.pad_token)
        test_lm_trues = make_tokens(test_lm_trues, self.tokenizer, eos_token=self.args.sep_token, pad_token=self.args.pad_token)
...
class CopyMechanism(nn.Module):
    ...
    def forward(self, context_vecs, hidden, trg_embs, vocab_dists, attn_dists, src_ids):  # (B, d_h), (B, d_h), (B, d_h), (B, V), (B, S_L), (B, S_L)
        ...
        final_dists = vocab_dists.scatter_add(dim=-1, index=src_ids, src=attn_dists)  # (B, V)
