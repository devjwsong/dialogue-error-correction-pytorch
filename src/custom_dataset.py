    ...
    def __init__(self, args, data_type, tokenizer):
        ...
        for class_label, obj in data.items():
            ...
            for i in tqdm(range(len(src_states))):
                ...
                src_seq = f"{bos_token} {state_seq} {sep_token} {src_utter} {eos_token}"
                src_tokens = tokenizer.tokenize(src_seq)
                src_ids = tokenizer.convert_tokens_to_ids(src_tokens)
                
                trg_seq = f"{bos_token} {trg_utter} {eos_token}"
                trg_tokens = tokenizer.tokenize(trg_seq)
                ...
                if len(trg_ids) > args.max_decoder_len:
                    trg_ids = trg_ids[args.max_encoder_len]
